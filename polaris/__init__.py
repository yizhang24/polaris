...
if __name__ == "__main__":
    ...
    while True:
        ...
        if calibration_controller.get_calibration_mode(configuration.device):
            ...
            cv2.aruco.drawDetectedMarkers(frame, [m.corners for m in markers], np.fromiter(
                (m.tag_id for m in markers), dtype=int))
            observations2d = [FiducialObservation2d(
                m.tag_id, m.corners) for m in markers]
