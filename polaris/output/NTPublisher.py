...
class NTPublisher:
    ...
    def send(self, config: Device, timestamp: float, observation: CameraPoseObservation3d, fps: int):
        if not self._initialized:
            table = ntcore.NetworkTableInstance.getDefault().getTable("/%s/output" %
                                                                      config.device_id)
            self._observations_publisher = table.getDoubleArrayTopic("observations").publish(
        ...
        if observation != None:
            ...
            observation_data.append(
                observation.pose.rotation().getQuaternion().W())
            observation_data.append(
                observation.pose.rotation().getQuaternion().X())
            observation_data.append(
                observation.pose.rotation().getQuaternion().Y())
            observation_data.append(
                observation.pose.rotation().getQuaternion().Z())
