...
class ConfigurationRetriever:
    ...
    def updateLocal(self, configuration: PolarisConfiguration):
        ...
        calib_data = cv2.FileStorage(
            self.CALIBRATION_FILENAME, cv2.FileStorage_READ)
        camera_matrix = calib_data.getNode("camera_matrix").mat()
        distort_coeffs = calib_data.getNode("distortion_coefficients").mat()
        if type(camera_matrix) == numpy.ndarray and type(distort_coeffs) == numpy.ndarray:
    ...
    def updateNT(self, configuration: PolarisConfiguration) -> None:
        if not self._nt_initialized:
            ...
            self._camera_id_sub = nt_table.getDoubleTopic(
                "camera_id").subscribe(configuration.camera.id)
            self._camera_resolution_width_sub = nt_table.getDoubleTopic(
                "camera_resolution_width").subscribe(configuration.camera.res_w)
            self._camera_resolution_height_sub = nt_table.getDoubleTopic(
                "camera_resolution_height").subscribe(configuration.camera.res_h)
            self._camera_auto_exposure_sub = nt_table.getDoubleTopic(
                "camera_auto_exposure").subscribe(configuration.camera.auto_exposure)
            self._camera_exposure_sub = nt_table.getDoubleTopic(
                "camera_exposure").subscribe(configuration.camera.exposure)
            self._camera_gain_sub = nt_table.getDoubleTopic(
                "camera_gain").subscribe(configuration.camera.gain)
            self._fiducial_size_m_sub = nt_table.getDoubleTopic(
                "fiducial_size_m").subscribe(configuration.environment.tag_size_m)
        ...
        configuration.camera.res_w = math.floor(
        ...
        configuration.camera.exposure = math.floor(
            self._camera_exposure_sub.get())
        ...
        try:
            configuration.environment.tag_map = json.loads(
