...
class PoseEstimator:
    ...
    def solve_camera_pose(self, observations: List[FiducialObservation2d], configuration: PolarisConfiguration, intrinsics: Intrinsics) -> CameraPoseObservation3d:
        ...
        for observation in observations:
            ...
            corner_0 = tag_pose + \
                Transform3d(Translation3d(0, tag_size / 2.0, -
                            tag_size / 2.0), Rotation3d())
            corner_1 = tag_pose + \
                Transform3d(Translation3d(0, -tag_size /
                            2.0, -tag_size / 2.0), Rotation3d())
            corner_2 = tag_pose + \
                Transform3d(Translation3d(0, -tag_size / 2.0,
                            tag_size / 2.0), Rotation3d())
            corner_3 = tag_pose + \
        ...
        if len(tag_ids) == 1:
            object_points = np.array([[-tag_size / 2.0, tag_size / 2.0, 0.0],
                                      ...
                                      [-tag_size / 2.0, -tag_size / 2.0, 0.0]])
            try:
                _, rvecs, tvecs, errors = cv2.solvePnPGeneric(object_points, np.array(
                    all_image_points), intrinsics.camera_matrix, intrinsics.distortion_coefficients, flags=cv2.SOLVEPNP_IPPE_SQUARE)
            except:
            ...
            return CameraPoseObservation3d(tag_ids, field_to_camera, errors[0][0], field_to_camera_alt, errors[1][0])
        else:
            ...
            try:
                _, rvecs, tvecs, errors = cv2.solvePnPGeneric(numpy.array(all_object_points), numpy.array(all_image_points),
                                                              intrinsics.camera_matrix, intrinsics.distortion_coefficients, flags=cv2.SOLVEPNP_SQPNP)
            except:
                return None
            ...
            # Return result
            return CameraPoseObservation3d(tag_ids, field_to_camera_pose, errors[0][0], None, None)
